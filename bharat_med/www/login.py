...
def get_context(context):
	...
	context["disable_user_pass_login"] = cint(frappe.get_system_settings("disable_user_pass_login")) or True
...
def send_login_link(email: str):
	...
	expiry = convert_minutes_to_hours_minutes(expiry)

	app_name = (
		frappe.get_website_settings("app_name") or frappe.get_system_settings("app_name") or _("Frappe")
	)
	if (frappe.db.exists("Email Template",{'name':'Email Login Access Template',"enabled": 1})):
		email_temp = frappe.get_doc("Email Template", "Email Login Access Template")
		mg = frappe.get_doc("mGrant Settings")
		donor = frappe.get_doc("Donor", mg.primary_donor)
		full_name = frappe.get_value("SVA User", {"email": email}, ["full_name"])
		html_temp = frappe.render_template(email_temp.response_html, {"link": link, "expiry": expiry, "donor":donor,"full_name":full_name})
		subject = frappe.render_template(email_temp.subject, {"donor":donor})
		frappe.sendmail(
			subject=subject,
			recipients=email,
			message=html_temp,
...
def convert_minutes_to_hours_minutes(m):
    if m < 60:
        return f"{m} minutes"
    else:
        hours = m // 60
...
def _generate_temporary_login_link(email: str, expiry: int):
	...
	return get_url(f"/api/method/bharat_med.www.login.login_via_key?key={key}")
