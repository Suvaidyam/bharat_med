...
login.bind_events = function () {
	...
	$(".form-login-with-email-link").on("submit", function (event) {
		event.preventDefault();
		var args = {};
		args.cmd = "bharat_med.www.login.send_login_link";
...
login.route = function () {
	...
	if(email){
		$("#login_with_email_link_email").val(email);
	}
...
login.reset_sections = function (hide) {
	if (hide || hide === undefined) {
		$("div.login-container").toggle(false);
		$("section.for-email-login").toggle(false);
		$("div.forgot-box").toggle(false);
		$("div.for-login-with-email-link").toggle(false);
...
login.login = function () {
	...
	if(rt == "/admin"){
		login.set_status({{ _("Login") | tojson }}, 'blue');
	} else {
		login.set_status({{ _("Send login link") | tojson }}, 'blue');
	}
	$(".login-container").toggle(true);
}
...
login.forgot = function () {
	login.reset_sections();
	login.set_status({{ _("Reset Password") | tojson }}, 'blue');
	if ($("#login_email").val()) {
		$("#forgot_email").val($("#login_email").val());
	}
	$(".forgot-box").toggle(true);
	$("#forgot_email").focus();
}
...
login.set_status = function (message, color) {
	$('div:visible .custom-btn-primary').text(message)
	if (color == "red") {
		$('div:visible .forgot-box').addClass("invalid");
	}
}

login.set_invalid = function (message) {
	$(".mgrant-login-content.mgrant-page-card").addClass('invalid-login');
	setTimeout(() => {
		$(".mgrant-login-content.mgrant-page-card").removeClass('invalid-login');
	}, 500)
...
var request_otp = function (r) {
	$('.mgrant-login-content').empty();
	$('.mgrant-login-content:visible').append(
		`<div id="twofactor_div">
			<form class="form-verify">
				<div class="mgrant-page-card-head p-0">
...
var continue_sms = function (setup, prompt) {
	request_otp();
	var sms_div = $('<div class="text-mogin?redirect-to=%2Fapp%2Fbuilduted" style="padding-bottom: 15px;"></div>');
